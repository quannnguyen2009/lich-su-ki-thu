    ...
    <nav className='hidden md:flex space-x-6'>
      {navigationItems.map(item => (
        <SmoothScrollLink
          key={item.id}
          href={`#${item.id}`}
          className='text-gray-700 hover:text-[#BF2F1F] transition-colors duration-300 font-medium'
        >
          {item.label}
        </SmoothScrollLink>
      ))}
    ...
    <div className='flex flex-col space-y-4 py-4'>
      {navigationItems.map(item => (
        <SmoothScrollLink
          key={item.id}
          href={`#${item.id}`}
          className='text-gray-700 hover:text-[#BF2F1F] transition-colors duration-300 font-medium py-2'
        >
          {item.label}
        </SmoothScrollLink>
      ))}
